...
* **Core Schemas:**
  ...
  * `sorting_rules`: Item keys, bilingual descriptions, and WCC disposal instructions. The seed dataset (`db/seeds/02_sorting_rules.js`) is unverified placeholder content — drafted from the 2024 national kerbside standardisation and WCC's published guidance but not confirmed row-by-row against WCC's live pages, and the Te Reo Māori text awaits review by a fluent speaker — the same status as the schedule epoch pending real WCC calendar data (§2B).
