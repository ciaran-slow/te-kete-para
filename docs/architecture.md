...
* **Header & Language Toggle:** `src/components/language-toggle.tsx` renders a Radix `ToggleGroup` (`role="radiogroup"`, single-select `radio` items) inside `<header>` in `src/app/layout.tsx`, giving every route a persistent, accessible EN/Te Reo switch wired directly to `useTranslation()`. Shared, reusable UI components live in `src/components/`, one file per component (ADR 0011).
