...
* **Shared Status Announcements:** `src/components/status-region.tsx`
  ...
  messages, `ShiftAlertBanner`'s announcement `<div>`, and
  `LanguageToggle`'s locale-change announcement `<p>` (issue #152) all
...
* **Shift-Alert Banner:** `src/components/shift-alert-banner.tsx` renders
  ...
  0066). The nightly dispatch pipeline (§2B below) is now also gated on
  this same per-address data (ADR 0073, issue #144).
...
* **Core Schemas:**
  * `addresses`: a two-tier registry (issue #178, ADR 0075). Tier 1 is 17 hand-curated rows with a fully confirmed operational classification — council zones, suburb, CBD vs. suburban night collection (`is_inner_city_night_collection`), which of WCC's two independently-phased alternating recycling calendars the address follows (`recycling_calendar_group`, 1 or 2, `null` for CBD rows; confirmed per-address against WCC's live per-street lookup tool, ADR 0059, issue #102), and — for suburban rows — which real WCC weekday the address's weekly kerbside collection actually falls on (`collection_weekday`, 0–6 per `Date#getUTCDay()`, `null` for CBD rows and any address not yet confirmed; confirmed per-address against the same WCC live per-street lookup tool, ADR 0063, issue #117). Tier 2 is a much larger set of bulk-imported rows sourced from WCC's own street-search registry (`RubbishCollectionStreetsHandler.ashx`, ADR 0075), findable by name but with `zone: "zone-unconfirmed"` and `is_inner_city_night_collection`/`recycling_calendar_group`/`collection_weekday` all genuinely `null` — never guessed. `is_inner_city_night_collection` is therefore nullable (widened from `NOT NULL DEFAULT false` by ADR 0075): `null` is a real, distinct "not yet classified" state, exactly like the other two fields, and every consumer (`computeCollectionRuleSet` in `rules.ts`, `isCollectionDay`/`findNextCollectionDate` in `collection-day.ts`) throws `UnresolvedZoneClassificationError` rather than defaulting a bulk-imported row to a plausible-but-wrong suburban schedule. `src/lib/schedule/collection-day.ts` exports the pure `isCollectionDay`/`findNextCollectionDate` functions that consume this field (no DB access, ADR 0015), now wired into `<ScheduleDisplay>`/`<ShiftAlertBanner>` to restore a genuine per-address claim (ADR 0066, issue #134, superseding ADR 0019/ADR 0053). The nightly dispatch pipeline (§2B) now also gates its per-subscriber decision on this field (ADR 0073, issue #144).
...
* **Seed Idempotency:** `db/seeds/*.js` files that populate reference/lookup tables (starting with `db/seeds/01_addresses.js`) achieve idempotency by deleting all rows in the target table before reinserting the dataset (the 17 curated rows plus, for `addresses`, the bulk-imported rows from `db/seeds/data/wellington-streets.json`, ADR 0075), not by upserting on a natural key — `addresses` has no unique constraint to upsert against, and adding one is out of scope for a seed-only change. This is deliberately destructive: a re-run nulls any `users.address_id` / `push_subscriptions.address_id` that referenced a previously-seeded row, via the existing `ON DELETE SET NULL` foreign keys (ADR 0012). `01_addresses.js`'s bulk-row insert uses `knex.batchInsert` (chunked at 200 rows), not a single `insert()` call — sqlite3's `SQLITE_MAX_COMPOUND_SELECT` limit (default 500) rejects the multi-row `select ... union all select ...` knex's sqlite3 dialect builds for a single insert this large.
...
* **Coverage Enforcement:** `npm run test:coverage` (`vitest run --coverage`) measures product code only — `src/**/*.{ts,tsx}`, via `@vitest/coverage-v8` (ADR 0008) — and fails the run if **lines or statements** fall below **90%**; as of 2026-08-06, a run measured 98.21% statements / 99.17% lines (574 tests, 55 files) — a point-in-time snapshot from that command, not a permanent figure, since real coverage drifts with every change that touches `src/` and the only number that always holds is the enforced 90% floor. Branches and functions are reported in the CI log but not gated: `src/lib/db.ts` selects its Knex config on `NODE_ENV === "test"`, and Vitest always sets `NODE_ENV=test`, so the `development` side of that branch is unreachable from the suite (ADR 0008 §Trade-offs and consequences).
